...
void	apply_depth_shadow(t_app *fdf)
{
	...
	while (++i < total)
	{
		// crude pseudo-depth gradient using row
		depth_norm = (float)(i / fdf->width) / (float)(fdf->height - 1 + (fdf->height == 1));
...
void	apply_ambient_shadows(t_app *fdf)
{
	...
	while (++i < total)
	{
		// simple ambient falloff toward center
		int y = i / fdf->width;
		float ny = (float)y / (float)(fdf->height - 1 + (fdf->height == 1));
